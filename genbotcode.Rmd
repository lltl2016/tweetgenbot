...
if (new > 0) {
  ...
  test$name <- sapply(dms, function(x) as.vector(x$getSenderSN()))[c(seq(new))]
  test$text <- sapply(dms, function(x) as.vector(x$getText()))[c(seq(new))]
  
  # test to see if DM matches our search phrase, keep those names that do
  final <- as.data.frame(test[which(str_detect(str_to_lower(test$text), 
